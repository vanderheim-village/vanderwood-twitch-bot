...
class BasicCommandsCog(commands.Cog):
    ...
    @app_commands.command(
        name="help", description="Get a list of commands which this bot supports."
    )
    ...
    @app_commands.command(
        name="standings",
        description="Get the current clan standings for the current season of the Battle of Midgard.",
    )
    async def standings(self, interaction: discord.Interaction) -> None:
        ...
        if await Channel.get_or_none(discord_server_id=interaction.guild.id):
            ...
            if await Season.active_seasons.all().filter(channel=channel).exists():
                ...
                logger.info(
                    f"There are no active seasons for this discord server ({interaction.guild.id})."
                )
                await interaction.response.send_message("There are no active seasons.")
        else:
            logger.info(
                f"This discord server ({interaction.guild.id}) has not been registered yet."
            )
            await interaction.response.send_message(
                "This discord server has not been registered yet."
            )

    @app_commands.command(
        name="checkin-leaderboard",
        description="Get the checkin leaderboard for the twitch channel.",
    )
    ...
    @app_commands.command(
        name="raid-checkin-leaderboard",
        description="Get the raid checkin leaderboard for the twitch channel.",
    )
    ...
    @app_commands.command(
        name="gifted-subs-leaderboard",
        description="Get the gifted subs leaderboard for the twitch channel.",
    )
    async def gifted_subs_leaderboard(self, interaction: discord.Interaction) -> None:
        ...
        if await GiftedSubsLeaderboard.all().exists():
            ...
            await interaction.response.send_message(
                "There are no entries in the gifted subs leaderboard."
            )

    @app_commands.command(
        name="watchtime-leaderboard",
        description="Get the watchtime leaderboard for the Twitch channel.",
    )
    async def watchtime_leaderboard(self, interaction: discord.Interaction) -> None:
        ...
        if await PlayerWatchTime.all().exists():
            ...
            await interaction.response.send_message(
                "There are no entries in the watchtime leaderboard."
            )

    @app_commands.command(
        name="lifetime-standings",
        description="Get the lifetime clan standings for the Battle of Midgard.",
    )
    ...
    @app_commands.command(
        name="leaderboard",
        description="Get the individual leaderboard for the current season of the Battle of Midgard.",
    )
    async def leaderboard(self, interaction: discord.Interaction) -> None:
        ...
        if await Channel.get_or_none(discord_server_id=interaction.guild.id):
            ...
            if await Season.active_seasons.all().filter(channel=channel).exists():
                ...
                embed = discord.Embed(
                    title=f"Battle of Midgard Individual Leaderboard | {season.name}:"
                )
                embed.timestamp = interaction.created_at
                embed.set_footer(
                    text=f"Requested by {interaction.user.name}",
    ...
    @app_commands.command(
        name="get-clan-player-counts",
        description="Get the player counts for each clan in the Battle of Midgard.",
    )
    @commands.guild_only()
    async def get_clan_player_counts(self, interaction: discord.Interaction) -> None:
        ...
        if await Channel.get_or_none(discord_server_id=interaction.guild.id):
            ...
            for clan in clans:
                ...
                embed.add_field(
                    name=f"{clan.name.title()}", value=f"{player_count} players", inline=True
                )
            await interaction.response.send_message(embed=embed)
        else:
            await interaction.response.send_message(
    ...
    @app_commands.command(
        ...
        description="Get the players for a specific clan in the Battle of Midgard.",
    )
    @commands.guild_only()
    async def get_clan_players(self, interaction: discord.Interaction, clantag: str) -> None:
        ...
        if await Channel.get_or_none(discord_server_id=interaction.guild.id):
            ...
            if await Clan.all().filter(channel=channel, tag=clantag).exists():
                ...
                await interaction.response.send_message(
                    f"Clan {clantag} does not exist.", ephemeral=True
                )
        else:
            await interaction.response.send_message(
                "This discord server has not been registered yet.", ephemeral=True
            )

    @app_commands.command(
        name="check-player-clan",
        description="Check the clan of a specific player in the Battle of Midgard.",
    )
    @commands.guild_only()
    async def check_player_clan(self, interaction: discord.Interaction, player_name: str) -> None:
        ...
        if await Channel.get_or_none(discord_server_id=interaction.guild.id):
            ...
            if await Player.all().filter(channel=channel, name=player_name).exists():
                ...
                await interaction.response.send_message(
                    f"{player.name} is in {clan.name} [{clan.tag}]."
                )
            else:
                await interaction.response.send_message(
                    f"Player {player_name} does not exist.", ephemeral=True
                )
        else:
            await interaction.response.send_message(
                "This discord server has not been registered yet.", ephemeral=True
    ...
    @app_commands.command(
        ...
        description="Get the lifetime individual leaderboard for the Battle of Midgard.",
    )
